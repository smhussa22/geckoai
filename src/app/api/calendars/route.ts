...
export async function GET() { 
  ...
  try {
    ...
    const googleCalendarIds: string[] = googleCalendars.map((calendar: any) => calendar.id as string);

    const existing = googleCalendarIds.length ? await prisma.calendar.findMany({
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!googleResponse.ok) return NextResponse.json({ error: "METHOD: Calendars/POST, ERROR: Calendar API Response Failed" }, { status: googleResponse.status });
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    if (!dbCalendarId) { return NextResponse.json({ error: "METHOD: Calendars/DELETE, ERROR: calendarId (DB id) required" }, { status: 400 });}
    ...
    if (!dbCalendar) return NextResponse.json( { success: true, calendarId: dbCalendarId, mode: "not_found_locally" }, { status: 200 });
    
    const googleId = dbCalendar.googleId;
    if (googleId.toLowerCase() === "primary") return NextResponse.json( { error: "METHOD: Calendars/DELETE, ERROR: Primary calendar cannot be deleted" }, { status: 403 });
    ...
    if (responseMetaData.status === 404) {
      ...
      return NextResponse.json( { success: true, calendarId: dbCalendarId, mode: "not_found_on_google" }, { status: 200 });

    }
    if (!responseMetaData.ok) return NextResponse.json({ error: `METHOD: Calendars/DELETE, ERROR: Failed to fetch calendar` }, { status: responseMetaData.status });
    ...
    const endpoint = isOwner ? `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(googleId)}`: `https://www.googleapis.com/calendar/v3/users/me/calendarList/${encodeURIComponent(googleId)}`;
    ...
    if (!deletion.ok) return NextResponse.json( { error: `METHOD: Calendars/DELETE, ERROR: Failed to delete calendar` }, { status: deletion.status });

    await prisma.calendar.deleteMany({ where: { ownerId: userId, id: dbCalendarId } });

    return NextResponse.json( { success: true, calendarId: dbCalendarId, mode: isOwner ? "permanent" : "unsubscribe" }, { status: 200 });
