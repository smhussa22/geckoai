...
export async function GET() {
    ...
    try {
        ...
        const googleCalendars = Array.isArray(calendarListData?.items)
        ...
        const mergedCalendars = googleCalendars.map((google: any) => {
            ...
            return {
                ...
                backgroundColor:
                    pref?.color ?? google.backgroundColor ?? defaults.listBackgroundColor,
...
export async function DELETE(req: Request) {
    ...
    if (!session)
        return NextResponse.json(
            {
                error: "METHOD: Calendars/DELETE, ERROR: Error reading session/User may not be logged in; please log in again by refreshing",
            },
            { status: 401 }
        );
        ...
        return NextResponse.json(
            {
                ...
                mode: isOwner ? "permanent" : "unsubscribe",
            },
            { status: 200 }
        );
    } catch (error: any) {
