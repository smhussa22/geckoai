...
export async function GET(req: Request, { params }: { params: { calendarId: string } }) {
  ...
  try {
    ...
    const gUrl = new URL(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendar.googleId)}/events`);
    ...
    if (!gRes.ok) return NextResponse.json({ error: "METHOD: Events/GET, ERROR: Calendar API Response Failed"}, { status: gRes.status });
...
export async function POST(req: Request, { params }: { params: { calendarId: string } }) {
  ...
  try {
    ...
    if (!start || !end) return NextResponse.json({ error: "METHOD: Events/POST, ERROR: Start & End time required" }, { status: 400 });
    ...
    const gRes = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendar.googleId)}/events`,
