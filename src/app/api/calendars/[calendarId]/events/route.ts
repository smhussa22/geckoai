...
function buildRRule(rec: any): string | null {
  if (!rec || !rec.frequency || rec.frequency === "NONE") return null;

  let rrule = `FREQ=${rec.frequency.toUpperCase()}`;
  if (rec.interval) rrule += `;INTERVAL=${rec.interval}`;
  if (Array.isArray(rec.byDay) && rec.byDay.length > 0) {
    rrule += `;BYDAY=${rec.byDay.map((d: string) => d.toUpperCase()).join(",")}`;
  }
  if (rec.until) {
    const untilISO = new Date(rec.until).toISOString();
    const formatted = untilISO.replace(/[-:]/g, "").split(".")[0] + "Z";
...
export async function POST(req: Request, ctx: { params: Promise<{ calendarId: string }> }) {
  ...
  try {
    ...
    const { title, description, location, start, end, timeZone, recurrence } = body ?? {};
    
    // Handle missing start/end times with defaults
    ...
    if (start) {
      ...
      return NextResponse.json(
        { error: "METHOD: Events/POST, ERROR: Start time required" },
        { status: 400 }
      );
    }
    ...
    if (end) {
      ...
      return NextResponse.json(
        { error: "METHOD: Events/POST, ERROR: End time required" },
        { status: 400 }
      );
    }
    ...
    if (!gRes.ok) {
      ...
      return NextResponse.json(
        { error: "METHOD: Events/POST, ERROR: Calendar API Response Failed", details: errorText },
        { status: gRes.status }
      );
    }
