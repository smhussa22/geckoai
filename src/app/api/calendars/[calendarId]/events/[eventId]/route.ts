...
export async function GET(
  ...
  if (!session)
    return NextResponse.json(
      { error: 'METHOD: Event/GET, ERROR: Could not retrieve cookie session' },
      { status: 401 },
    );
  ...
  try {
    ...
    if (!googleToken)
      return NextResponse.json(
        { error: 'METHOD: Event/GET, ERROR: No Google Token' },
        { status: 401 },
      );
    ...
    if (!gRes.ok)
      return NextResponse.json(
        { error: 'METHOD: Event/GET, ERROR: Calendar API Response Failed' },
        { status: gRes.status },
      );
    ...
    return NextResponse.json(
      { error: error?.message || 'METHOD: Event/GET, ERROR: Internal Service Error' },
      { status: 500 },
    );
  }
}
