        ...
        <div className="p-4">
          <div
            ...
            className={`grid h-40 place-items-center rounded-xl border-2 border-dashed transition-colors ${
              isDragging ? 'border-asparagus bg-asparagus/10' : 'border-neutral-700 bg-neutral-900'
            } cursor-pointer`}
          >
            <div className="flex flex-col items-center text-neutral-300">
              <RiUploadCloud2Line size={48} className="mb-2" />
              <div className="text-sm">
                <span className="font-medium text-neutral-100">Drag & drop</span> or click to browse
              </div>
              <div className="mt-1 text-xs text-neutral-400">
                Accepted: {acceptableFiles} • Max {formatToMegabytes(maxFileSizeInBytes)} MB • Up to{' '}
          ...
          {staged.length > 0 && (
            <div className="mt-4">
              <div className="mb-2 flex items中心 justify-between">
                <div className="text-sm text-neutral-300">Staged ({staged.length})</div>
                <button
                  onClick={clearAll}
                ...
                {staged.map((f, i) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="truncate text-sm text-neutral-100">{f.name}</div>
                      <div className="text-xs text-neutral-400">
                        {formatToMegabytes(f.size)} MB • {extensionOf(f.name) || 'unknown'}
                      </div>
                    </div>
                    <button
                      onClick={() => removeAt(i)}
