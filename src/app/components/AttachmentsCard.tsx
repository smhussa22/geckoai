...
export default function AttachmentsCard({ items = [], onRemove, className = "" }: { items?: string[]; onRemove?: (index: number) => void; className?: string; }) {
      ...
      <div className="flex flex-wrap gap-2 justify-start">
        ...
        {items.map((label, i) => {
          ...
          return (
            ...
            <div className="group relative inline-flex items-center gap-2 max-w-60 rounded-md border border-neutral-700 bg-neutral-900 px-2 py-2 text-xs text-ghost" key={`${label}-${i}`} title={label} >

              <span className="shrink-0 grid place-items-center p-1">
              ...
              <span className="truncate text-sm">{label}</span>
                ...
                <button type="button" onClick={handleDelete(i, onRemove)}>
                  ...
                  <CiCircleRemove size={24} className="text-neutral-400 hover:text-neutral-200 transition-colors drop-shadow" />
