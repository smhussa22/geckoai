...
function GridEvent({ event }: { event: CalendarEvent }) {
    ...
    return (
        <div className="pointer-events-none leading-tight select-none">
            <div className="truncate font-semibold">{event.title}</div>
            <div className="text-xs opacity-90">{timeStr}</div>
            {event.description ? (
                <div className="mt-0.5 line-clamp-2 text-xs opacity-80">{event.description}</div>
            ) : null}
        </div>
    );
}
...
export default function BigCalendarRemote() {
    ...
    const fetchEventsForRange = useCallback(
        async (start?: Date, end?: Date) => {
            ...
            try {
                const base = new URL(
                    `/api/calendars/${calendar.id}/events`,
    ...
    return (
        ...
        >
            {errorMsg ? <div style={{ marginBottom: 8, color: "#f87171" }}>{errorMsg}</div> : null}
