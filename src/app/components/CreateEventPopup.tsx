...
export default function CreateEventPopup({ onClose }: { onClose?: () => void }) {
	...
	const endDefault = useMemo( () => new Date(startDefault.getTime() + 60 * 60 * 1000), [startDefault]);
	...
	const [startTime, setStartTime] = useState(`${pad(startDefault.getHours())}:${pad(startDefault.getMinutes())}`);
	const [endTime, setEndTime] = useState(`${pad(endDefault.getHours())}:${pad(endDefault.getMinutes())}`);
  	...
  	return (
    	...
    	<div className="w-fit" data-user-email={user?.email || ""}>
      		...
      		<div className="flex relative">
        		...
        		<div>
          			...
          			<h2 className="mb-2 font-semibold tracking-tighter text-broccoli">Only date and time are required.</h2>
		...
		<div className="mt-2 flex gap-3">
		  		...
		  		<div className="flex-1 space-y-2">
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						<input
							...
							className="rounded-md text-ghost bg-neutral-800 px-3 py-2 text-sm hover:bg-neutral-700/70 outline-none border border-transparent focus:border-neutral-700"
						/>
      	...
      	<div className="mt-4 flex items-center justify-end">
			...
			className="group hover:text-broccoli hover:bg-night duration-200 flex items-center justify-center text-2xl tracking-tighter w-full rounded-lg bg-broccoli text-night transition-colors font-bold py-2 px-4 disabled:opacity-60 disabled:cursor-not-allowed"
			>
