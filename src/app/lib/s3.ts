...
export async function s3WriteMessageJSON(
  ownerId: string,
  calendarId: string,
  messageId: string,
  body: any,
...
export function s3StagedAttachmentKey(
  ownerId: string,
  calendarId: string,
  tempId: string,
  filename: string,
...
export function s3CommittedAttachmentKey(
  ownerId: string,
  calendarId: string,
  messageId: string,
  filename: string,
...
export async function s3DeletePrefix(prefix: string) {
  ...
  do {
    ...
    if (keys.length > 0)
      await s3.send(new DeleteObjectsCommand({ Bucket: s3Bucket, Delete: { Objects: keys } }));
